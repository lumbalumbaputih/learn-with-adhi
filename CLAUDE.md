...
- **Akuarium Raya 🐠** (`courses/games/raya-akuarium-v1.html`, key `rayaAkuarium`) – ✅ **3D aquarium game** (2026-06-26): Raya's **first 3D star-earning game** (the 3 existing 3D games are family/front-door only; this one is hers and counts toward her stars). Tap-to-pick sea creatures in a vendored-Three.js r128 tank (Raycaster object-picking + big transparent hit-spheres = toddler-forgiving tap targets; same engine pattern as Robot Rapi). **6 no-fail, audio-guided levels:** Sentuh Ikannya (free play) → Ayo Hitung (count 1–5, numbers spoken) → Cari Warna (color-match, color-swatch cue) → Besar & Kecil (size) → Teman Laut (find gurita/kepiting/kura-kura/bintang laut) → BOSS Pesta Akuarium (count + color + find, 3D confetti). **Built for a non-reader:** color swatches + emoji hints + `id-ID` read-aloud (`Suara`) + WebAudio `SFX` carry ALL meaning (no instruction depends on reading); wrong tap = gentle fish wiggle + spoken redirect, **never "Salah!"**, **full stars on completion** (no fail state, per the v3 no-fail rule). Play area auto-fits the visible camera frustum (`computeBounds()`) so **no fish swims off-screen on a narrow portrait phone** — found & fixed via a headless projection test. OrbitControls (tap-vs-drag rejection), WebGL fallback, 🔇 mute (`rayaAkuariumMuted`), `lastPlayed` stamp for the parent dashboard. Wired in **all 6 places**: `raya.html` (GAMES, with progress + star count), `kids.html` (`sumStars` + pill), `auth.js` (`PROGRESS_KEYS_BY_TYPE`), `dashboard.html` (`COURSE_NAMES`), `sw.js` (`PRECACHE` + the vendored `three.min.js`/`OrbitControls.js`, `CACHE_VERSION`→`lwa-v2`), `index.html` kids-strip (3→4 game). 🧪 needs a real-device WebGL/touch pass.
- **Kereta Raya 🚂** (`courses/games/raya-kereta-v1.html`, key `rayaKereta`) – ✅ **3D train game** (2026-06-26): Raya's 2nd 3D star-game, themed to the **family KAI train world** (the "one connected world" principle — Kiki rides the same train; ties Raya's world to her brother's train biome). **Distinct mechanic from the aquarium:** *sorting/pairing* — tap a piece of cargo → tap the matching wagon (vendored Three.js r128 + Raycaster, big hit-spheres). Cute KAI locomotive (face, chimney, cowcatcher, spinning wheels) + open-top colored wagons + station + scenery. **6 no-fail, audio-guided levels:** Muat Keretanya (free load) → Kereta Warna (color-match, the matching wagon **glows** as a no-read hint) → Hitung Penumpang (count animal passengers boarding, spoken) → Siapa Naik? (find/board the named animal) → Muatan Bentuk (shape-match: bulat/kotak/bintang) → **BOSS** Jalan-Jalan! (color-sort a 4-wagon train → a long scenic **departure ride**, camera follows + confetti). **The reward every level is the train chugging off on a 3D ride** (whistle "tut tut!", wheels spin, scenery parallax). Non-reader-first: color swatches + shape/animal emoji cues + `id-ID` read-aloud + WebAudio; wrong wagon = gentle bump + spoken redirect, **never "Salah!"**, **full stars** on completion. `fitCamera()` frames the whole train for any aspect (no wagon off-screen on portrait); **once a cargo is held, taps prefer wagon hits** so foreground cargo can't steal a wagon tap. WebGL fallback, 🔇 mute (`rayaKeretaMuted`), `lastPlayed` stamp. Verified by headless play-through of **all 6 levels** (both interaction modes + the ride + star persistence → 18★, zero errors). Wired in all 6 places (`sw.js` `CACHE_VERSION`→`lwa-v3`; `index.html` kids-strip 4→5 game). 🧪 needs a real-device WebGL/touch pass.
...
- **Ular Tangga 3D 🎲** (`courses/games/ular-tangga-v1.html`, id `ular-tangga-game`) – ✅ **3D classic Snakes & Ladders** (2026-06-13): the universally-known board game in real 3D (vendored Three.js r128, no new deps). 10×10 numbered board, 7 arcing tube ladders + 8 wavy snakes (eyed heads), tumbling 3D dice, color pawns that hop tile-by-tile / climb / slide, OrbitControls + 🔭 top-down toggle, WebAudio SFX + mute. 2–4 players local hotseat, classic rules (6 = roll again, overshoot bounces back, first to 100 wins → 🏆 + 3D confetti). Bahasa Indonesia. **Family game, NOT a per-kid star course** → manifest front door only (`assets/courses.json`, category **"Play"**); not in `kids.html`/`sumStars`/`sw.js`. 🧪 needs a real-device WebGL/touch pass.
...
- **Robot Rapi 🤖✨** (`courses/games/robot-rapi-v1.html`, id `robot-rapi-game`) – ✅ **3D file-organization game** (2026-06-17): teaches **digital file organization** (a real digital-literacy skill) disguised as helping a friendly robot tidy its data warehouse. Vendored Three.js r128 + a tap-to-pick `Raycaster` layer (the first 3D game here to do object picking; the others are dice-button only). Core mechanic = **tap a file → tap a folder**, with arc-fly + sparkle on a match and a gentle folder-shake on a miss (**no "Salah!", no fail state**, per the v3 no-punishment rule). 6 levels: (1) berkas vs folder → (2) sort by type → (3) clear file names → (4) sub-folders / paths (`Foto › Liburan`) → (5) *find* a file by opening folders → (6) BOSS: clean a messy computer to 100% Kerapian → 🏆 + 3D confetti. Watching robot mascot cheers, ✨ tidiness meter, ⭐ stars-by-attempts (local-only `robotRapiProgress`), 🗂️ level select, WebAudio SFX + 🔇 mute, OrbitControls, WebGL fallback. Bahasa Indonesia UI + real tech words (Berkas/Folder). **Built at Adhi's request as a *family game* (front door only)** — wired on the manifest (`assets/courses.json`, category **"Play"**, n `G3`) + sitemap; deliberately *not* in `rayyan.html`/`kids.html`/`sumStars`/`auth.js`/`dashboard.html`/`sw.js` (so it does NOT count toward Rayyan's stars). 🧪 needs a real-device WebGL/touch pass.
