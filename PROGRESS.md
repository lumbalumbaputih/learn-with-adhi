...
- ✅ **NEW 3D game for Raya (her 2nd): "Kereta Raya 🚂"** (`courses/games/raya-kereta-v1.html`, key `rayaKereta`) — Adhi loved the aquarium and asked for **another game for Raya**. Built a 3D KAI-style train where she loads the wagons, then drives it on a scenic ride.
  - **Why this one:** deliberately *distinct* from the aquarium — a different **mechanic** (sorting/pairing: tap cargo → tap the matching wagon, vs the aquarium's tap-to-identify) and a different **theme** that ties Raya's world to the **family train universe** (the "one connected world" rule — Kiki rides the same KAI line, her brother's whole math ladder starts on the train).
  - **Engine:** reused the proven aquarium scaffolding (vendored Three.js r128 + Raycaster picking, big transparent hit-spheres, tweens, particles, no-fail flow, audio modules). New 3D: a cute KAI locomotive (face/chimney/cowcatcher/spinning wheels), open-top colored wagons, a station + parallax scenery (trees/hills/clouds), a long track.
  - **Built for a non-reader:** every instruction = a **color swatch / shape symbol / animal emoji** + `id-ID` read-aloud + WebAudio. In the color & shape levels the **matching wagon glows** so she can pair purely by sight. Counting is spoken ("satu… dua…").
  - **6 no-fail levels:** Muat Keretanya (free load) → Kereta Warna (color-match) → Hitung Penumpang (count passengers boarding) → Siapa Naik? (find/board the named animal) → Muatan Bentuk (shape-match 🔵🟥⭐) → **BOSS** Jalan-Jalan! (sort a 4-wagon train → a long scenic departure ride + confetti). **Every level ends with the train chugging off on a 3D ride** (whistle, wheels spin, camera follows, scenery scrolls) — the payoff that makes it a *train* game. Wrong wagon = gentle bump + spoken redirect, **never "Salah!"**; **full stars** on completion. Saved in the standard `{totalStars, progress:{l0..l5}}` shape + `lastPlayed`.
  - **Testing (headless Chromium / swiftshader WebGL):** confirmed init + on-screen projection (a `fitCamera()` that frames the whole train for any aspect — so no wagon goes off-screen on portrait, the same class of bug the aquarium had). First play-through caught a **real UX bug**: foreground cargo could sit between the camera and a wagon, stealing a wagon tap — fixed so that **once a cargo is held, taps prefer wagon hits**. Re-ran and **played all 6 levels to completion** (both interaction modes + the ride), all 3★, persisted to 18★, zero page errors. (Temporary `#qa` hook removed before commit; `node --check` clean; no debug artifacts.)
  - **Wired (per-kid star game, all 6 places):** `raya.html` (GAMES + progress + star count; "2 game"→"3 game"), `kids.html` (`sumStars` + "🎮 3 game" pill), `assets/js/auth.js` (cloud sync), `dashboard.html` (`COURSE_NAMES`), `sw.js` (`PRECACHE`; `CACHE_VERSION` `lwa-v2`→`lwa-v3`), `index.html` kids-strip (4→5 game). Raya now has **3 games** (Akuarium 🐠, Kereta 🚂, Cari 🔍) + 7 courses.
  - 🧪 **Next:** real-device WebGL/touch pass on the tablet (tap-pick accuracy on the train, `id-ID` voice, load time).

- ✅ **NEW 3D game for Raya: "Akuarium Raya 🐠"** (`courses/games/raya-akuarium-v1.html`, key `rayaAkuarium`) — Adhi asked for **a 3D game for Raya**, with a free hand on the kind, as long as it suits a **kid who can't read yet**. Built a single self-contained HTML file: a magic 3D aquarium where she taps cute sea creatures.
  - **Why this game:** Raya's whole catalog was quiz-shaped; this is a fresh **3D play format** that stealth-reviews her entire curriculum (counting, colors, sizes, sea animals) without ever feeling like a test — exactly the plan-v3 "story/play, not assessment" thesis, now in 3D.
  - **Engine:** vendored **Three.js r128 + OrbitControls** (no new deps) + **Raycaster** object-picking — the 2nd kid game to do tap-to-pick (after Robot Rapi). Each creature carries a big **transparent hit-sphere** so a 3-year-old's imprecise taps still land. Friendly low-poly builders for fish / octopus (gurita) / crab (kepiting) / turtle (kura-kura) / starfish (bintang laut), a tween engine, sparkle + ambient-bubble + confetti particles, swaying seaweed, sandy floor & rocks.
  - **Built for a non-reader (the core constraint):** every instruction is carried by **a color swatch or an emoji cue + `id-ID` read-aloud (`Suara`) + WebAudio `SFX`** — nothing depends on reading text. 🔊 "Dengar" button re-speaks the task. Counting is spoken aloud ("satu… dua… tiga!").
  - **6 no-fail levels:** Sentuh Ikannya (free play) → Ayo Hitung (count 1–5) → Cari Warna (match the swatch) → Besar & Kecil (size) → Teman Laut (find the named creature) → **BOSS** Pesta Akuarium (count + color + find → 3D confetti). **No fail state:** a wrong tap = gentle fish wiggle + a soft spoken redirect, **never "Salah!"**; completing a level always awards **full stars** (per the v3 no-fail rule). ⭐ stars saved in the standard `{totalStars, progress:{l0..l5}}` shape + a `lastPlayed` stamp.
  - **Testing (headless Chromium / Playwright + swiftshader WebGL):** confirmed Three.js inits (canvas, no fallback), the intro→level flow, and **played levels 1/3/5/6 to completion by tapping live-projected fish** — all award 3★ and persist correctly, zero page errors. A projection test caught a **real bug**: fish at world-x ±2.6 projected to screen-x 427 / −22 on a 414px portrait phone (off-screen → untappable). Fixed with `computeBounds()`, which fits the play area to the camera's visible frustum per device/orientation; re-tested = all creatures on-screen. (A temporary `#qa` debug hook used for testing was removed before commit; `node --check` clean; no debug artifacts left.)
  - **Wired as a per-kid star game (all 6 places):** `raya.html` (added to `GAMES`; the games grid now shows progress + counts game stars into the hub total; "1 game"→"2 game"), `kids.html` (`sumStars` + "🎮 2 game" pill), `assets/js/auth.js` (`PROGRESS_KEYS_BY_TYPE.raya`, for cloud sync), `dashboard.html` (`COURSE_NAMES`, parent view), `sw.js` (`PRECACHE` + the vendored `three.min.js`/`OrbitControls.js`; `CACHE_VERSION` bumped `lwa-v1`→`lwa-v2` so the new precache activates), `index.html` kids-strip (3→4 game). Unlike the 3 existing 3D games (Ular Tangga / Penyelamat Bumi / Robot Rapi, which are **family/front-door only**), this one is **Raya's** and counts toward her stars + shows in the parent dashboard.
  - 🧪 **Next:** real-device WebGL/touch pass on the actual tablet (tap-pick accuracy, `id-ID` voice availability, Web-Audio-after-gesture, load time) — same caveat as the other 3D games.
